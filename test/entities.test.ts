...
import { ChainId, WFLR as _WFLR, TradeType, Rounding, Token, TokenAmount, Pair, Route, Trade } from '../src'
...
const WFLR = _WFLR[ChainId.TESTNET]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WFLR, decimalize(1234, WFLR.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WFLR]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WFLR)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WFLR, decimalize(10, WFLR.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WFLR, '1662497915624478906')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WFLR, '1662497915624478906')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WFLR, '1234567000000000000000')
