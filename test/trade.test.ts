...
describe('Trade', () => {
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WFLR[ChainId.MAINNET], JSBI.BigInt(1000)),
  ...
  it('can be constructed with FLR as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], FLR),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(FLR)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with FLR as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], FLR, token0),
  ...
  it('can be constructed with FLR as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, FLR),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(FLR)
  })
  it('can be constructed with FLR as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, FLR),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for FLR currency input', () => {
      const result = Trade.bestTradeExactIn(
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(FLR)
      expect(result[0].route.path).toEqual([WFLR[ChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(FLR)
      expect(result[1].route.path).toEqual([WFLR[ChainId.MAINNET], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })
    it('works for FLR currency output', () => {
      const result = Trade.bestTradeExactIn(
        [pair_weth_0, pair_0_1, pair_0_3, pair_1_3],
      ...
      expect(result[0].route.path).toEqual([token3, token0, WFLR[ChainId.MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(FLR)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WFLR[ChainId.MAINNET]])
      expect(result[1].outputAmount.currency).toEqual(FLR)
    })
  })
